...
public class StockMarketJFrame extends JFrame {
    ...
    public StockMarketJFrame() {
        try {
            ...
            portfolio = new UserPortfolio(new BigDecimal(1000), new TreeMap<>(), marketService);
            portfolio.refreshPortofolioData();
    ...
    public void addStock(String symbol, int quantity) throws IOException {
        if (portfolio.getCash().compareTo(marketService.getStockPrice(symbol).multiply(new BigDecimal(quantity))) >= 0) {
            portfolio.getShares().put(symbol, quantity);
            portfolio.setCash(portfolio.getCash().subtract(marketService.getStockPrice(symbol).multiply(new BigDecimal(quantity))));
        } else {
            JOptionPane.showMessageDialog(this, "Not enough funds to buy " + quantity + " shares of " + symbol);
        }
        portfolio.refreshPortofolioData();
    }
